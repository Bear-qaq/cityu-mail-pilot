...
| 套件 | 断言 | 它盯住的东西 |
|---|---|---|
| `landing_check` | 55 | 落地→申请→批准→码真的能注册；禁 JS 可读；standalone 直通；右上角下载入口真的跳到位；安装步骤两个平台都在；没有安装包时绝不给死链；**首屏那句「装成一个应用」真的可见且加粗、且排在 `#how` 之前**（标记顺序证明不了一个访客看不看得见）；**`ol.steps` 每一步的第一个子元素都是 `<b>`**（顺序就是内容，得能扫）；**安装步骤的编号圆点真的画出来了**（`list-style:none` + `::before` 的 `content:counter(step)` + 非透明背景 + 22×22） |
