...
| `admin_edit_check` | 161 | 管理员代改另一个用户；**账号卡默认全收起 + 手风琴 + 勾选框按真实坐标点（不会顺手展开那一行）+ 刷新后还开着**；健康卡；巡检面板与**「已知晓」**（点的是**带冒号**的那条 key）；邮件面板；token 用量；审计列表；**替用户刷新状态**（全选/单个人都打同一个接口，夹具连不上就必须如实报 ✗，且不许点亮「出报告」）；**第三种提醒的模板与预览**（编辑框里是 `{steps}`，预览里才是学校那边的步骤）；设置向导第 2 步的转发结论**是画出来的**（不是藏 tooltip）。**需要 `--admin-fixtures`** |
